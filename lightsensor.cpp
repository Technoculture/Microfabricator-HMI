...
LightSensor::LightSensor(QObject *parent, ucSerial* serial) : QObject(parent), serial_(serial){
    connect(this, SIGNAL(updateSensorData(QString)), parent, SLOT(serialData(QString)));
}

void LightSensor::readIncommingData(){
    QStringList buffer_split = serialBuffer_.split(";");
    if(buffer_split.length() < 4){ // no parsed value yet so continue accumulating bytes from serial in the buffer.
